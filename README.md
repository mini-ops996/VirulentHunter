...
## 7. Usage with Docker (Local Inference)

For the easiest setup, we recommend using [Docker](https://www.docker.com/get-started/). This approach is helpful because it bundles all necessary dependencies (Python, PyTorch, CUDA, and the ESM models) into a single isolated environment, ensuring the tool runs reproducibly without requiring you to manually configure your local system libraries.

### Run Command

```bash
docker run --gpus all -v ${PWD}/data:/app/data -v ${PWD}/results:/app/results dwalke/virulenthunter:latest predict.py -i data/test.fasta -o results/ --base_esm_path models/esm2_t30_150M_UR50D
```

### Command Explanation

* **`--gpus all`**: Grants the container access to all available NVIDIA GPUs on your host machine. This is crucial for accelerating inference times.
* **`-v ${PWD}/data:/app/data`**: Mounts your local `data` directory (in your current working directory) to `/app/data` inside the container. This allows the tool to read your input FASTA files.
* **`-v ${PWD}/results:/app/results`**: Mounts your local `results` directory to `/app/results` inside the container. This ensures that the output files generated by the tool are saved to your local machine and persist after the container stops.
* **`dwalke/virulenthunter:latest`**: Specifies the image to use. If you do not have it locally, Docker will automatically download it.
* **`predict.py ...`**: The actual command running inside the container.
    * `-i`: The path to the input file (relative to the container's internal structure).
    * `-o`: The output directory.
    * `--base_esm_path`: Points to the specific ESM model architecture being used.
